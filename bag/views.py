    ...
    if size:
        if item_id in list(bag.keys()):
            if size in bag[item_id]['items_by_size'].keys():
                ...
                messages.success(request, f'Updated size {size} {product.name} to the bag')
            else:
                bag[item_id]['items_by_size'][size] = quantity
                messages.success(request, f'Added size {size} {product.name} to the bag')
        else:
            bag[item_id] = {'items_by_size': {size: quantity}}
            messages.success(request, f'Added size {size} {product.name} to the bag')
    else:
    ...
    try:
        ...
        if size:
            ...
            if not bag[item_id]['items_by_size']:
                bag.pop(item_id)
            messages.success(request, f'Removed size {size} {product.name} from the bag')
        else:
            bag.pop(item_id)
